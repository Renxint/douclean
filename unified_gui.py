...
class UpdateThread(QThread):
    ...
    def __init__(self, url, new_dir):
        super().__init__()
        self.url = url
        self.new_dir = new_dir

    def run(self):
        import zipfile, requests as req
        ...
        try:
            ...
            r = req.get(self.url, stream=True, timeout=600)
...
class UpdatePage(QWidget):
    ...
    def __init__(self):
        ...
        self.status_label.setStyleSheet("font-size:18px;font-weight:bold;color:#F1F5F9;")
        self.status_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(self.status_label)

        self.progress_bar = QProgressBar()
        self.progress_bar.setFixedSize(360, 10)
        self.progress_bar.setStyleSheet("QProgressBar{background:#12122A;border:1px solid #252550;border-radius:5px;} QProgressBar::chunk{background:#E11D48;border-radius:4px;}")
    ...
    def _on_progress(self, dl, total):
        if total > 0:
            ...
            self.detail_label.setText(f"{dl/1024/1024:.1f}MB / {total/1024/1024:.1f}MB")
        else:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.stack.addWidget(self.update_page)      # 4
        ...
        try:
            ...
            if compare_versions(remote, VERSION) > 0:
                ...
                if reply == QMessageBox.StandardButton.Yes and url:
                    self.stack.setCurrentIndex(4)  # 更新覆盖页
